...
class Transcribe {
	...
	public function replace()
	{
		...
		{
			if(!empty($all_vars_for_lang) && is_array($all_vars_for_lang)){
...
// check if a template exists for segment_1/segment_2/segment_3/segment_4/segment_5
	...
	/*
	* FUNCTION breadcrumb
	* Used to generate new breadcrumbs for Structure URL's  This is needed based on how we allow a given Structure Listing to be tied to multiple parents (one for each language)
	*
	 */
	public function breadcrumb()
	{
		...
		// get params
		$leading_slash = $this->EE->TMPL->fetch_param('leading_slash');
		$trailing_slash = $this->EE->TMPL->fetch_param('trailing_slash');

		// get current segments
		$current_segments = $this->EE->uri->uri_string;

		$current_segments = trim($current_segments, '/');
		$current_segments = explode('/', $current_segments);

		// get the data for this tree
		$return_data = $this->get_branch_data($current_segments, $leading_slash, $trailing_slash);
	...
	/**
	 * Function used to get current branch and parents data
	 * @param  array $current_segments  An array of the segments to get the branch for?
	 * @param  boolean $leading_slash  Include a leading slash on the crumb_url?
	 * @param  boolean $trailing_slash Include a trailing slash on the crumb_url
	 * @return [type]                  [description]
	 */
	public function get_branch_data($current_segments, $leading_slash = FALSE, $trailing_slash = FALSE)
	{
		$return_data = false;
		...
		{
				// set url as key... for lookup with current segments
			$uris = array_flip($current_pages[$this->site_id]['uris']);

			$num_current_segments = count($current_segments);

			for($i = 1; $i <= $num_current_segments; $i++)
			{
				// get the array segments needed and glue them with /
				$crumb_uri = implode('/', array_slice($current_segments, 0, $i));

				// do we have the structue page for this one?
				if(!empty($uris['/'.$crumb_uri]))
				{
					// get the array segments needed and glue them with /
					$return_data[$i-1]['crumb_uri'] = $crumb_uri;

					// assign entry_ids here to grab titles later... this saves a loop
					$entry_ids[] = $uris['/'.$return_data[$i-1]['crumb_uri']];

					// add the entry_id to the return_data
					$return_data[$i-1]['entry_id'] = $uris['/'.$return_data[$i-1]['crumb_uri']];

					// do we need to remove the leading slash?
					if( !empty($leading_slash) && strtolower($leading_slash) == 'yes')
					{
						$return_data[$i-1]['crumb_uri'] = '/'.$return_data[$i-1]['crumb_uri'];
					}

					// do we need to remove the trailing slash?
					if( !empty($trailing_slash) || strtolower($trailing_slash) == 'yes' )
					{
						$return_data[$i-1]['crumb_uri'] = $return_data[$i-1]['crumb_uri'].'/';
					}
					...
					if($i == 1)
					{
						$return_data[$i-1]['first'] = TRUE;
					}
					else
					{
						$return_data[$i-1]['first'] = FALSE;
					}

					// set the last variable
					if($i == $num_current_segments)
					{
						$return_data[$i-1]['last'] = TRUE;
					}
					else
					{
						$return_data[$i-1]['last'] = FALSE;
					}
				}
			...
			if(!empty($entry_ids))
			{
				// get titles for entry_ids
				$title_data = $this->EE->db->where_in('entry_id', $entry_ids)->select('title, entry_id')->get('channel_titles');

				if($title_data->num_rows > 0)
				{
					foreach($title_data->result() as $row)
					{
						// loop through return data
						foreach($return_data as $key => $return_row)
						{
							if($row->entry_id == $return_row['entry_id'])
							{
								// assign the title to the return data
								$return_data[$key]['title'] = $row->title;
								$return_data[$key]['breadcrumb:title'] = $row->title;
								// added second var to avoide issues with channel entries title field... title left in for legacy
							}
						}
					}
				}
	...
	public function _uri_reverse_lookup( $uri_string, $return = 0 )
	{
		...
		if( $this->EE->db->table_exists('structure') AND !empty($language_abbr) )
		{
			...
			if( !empty($_SESSION['transcribe']['uri_override']) )
			{
				$language_id = $_SESSION['transcribe']['uri_override'];
			}
			...
			else
			{
				...
				{
					// passing in true here will give us all of the structure tree... Otherwise it returns our modified version.
					$site_pages = $this->sql->get_site_pages(TRUE);
				}

				foreach($site_pages['uris'] as $key => $string)
				{
					if( $string == '/'.$language_abbr.'/' )
					{
						$site_pages['uris'][$key] = '/';
					}

					// are we dealing with an entry_id from the uri tag?
					if(!empty($this->EE->TMPL))
					{
						$passed_entry_id = $this->EE->TMPL->fetch_param('entry_id');
					}

					if(!empty($passed_entry_id) && ($passed_entry_id == $key) && $return == 1)
					{
						// if were passing in an entry id and we have a match.. go aehad and return the string.
						return $string;
					}
				}
			}
			...
			if( !empty($current_segment) && $current_segment{0} != '/')
				$current_segment = '/'.$current_segment;

			$reverse = strrev( $current_segment );
			if( !empty($reverse) && $reverse{0} != '/')
				$current_segment .= '/';
			...
			if( !empty($entry_ids) )
			{
				...
				// lets go ahead and make sure the current entry_ids are in our language
				$entry_ids = implode(",", $entry_ids);

				// go ahead and make sure their only in this langauge
				$new_entry_ids = $this->_get_language($entry_ids);
				unset($entry_ids);

				foreach( $new_entry_ids as $entry_id )
				{
					$entry_ids[] = $entry_id['entry_id'];
				}

				// rebuild the entry_ids
					...
					foreach($relationships->result_array() as $row)
					{
						if( $row['language_id'] == $language_id )
						{
							...
							if( !empty($site_pages['uris'][$new_page_entry_id]) )
							{
								...
								$redirect_url = $this->reduce_slashes($redirect_url);
		...
		if( ! Cache::get(array('lookup','entry', $lang_id)) )
		{
			...
			// fix for structure url issue... should be resolved elsewhere now
			// if(!empty($site_pages))
			// {
			// 	foreach ($entry_lookup as $row)
			// 	{
			// 		$uri = array_key_exists($row->entry_id, $site_pages['uris']) ? $site_pages['uris'][$row->entry_id] : NULL;

			// 		if($uri)
			// 		{
			// 			$slug = $uri == '/' ? '/' : end(explode('/', trim($uri, '/')));
			// 			$row->relationship = $slug;
			// 		}
			// 	}
			// }
