...
class Channel_images_ft extends EE_Fieldtype
{
    ...
    function save($data)
    {
        ...
        else
        {
            ...
            // -----------------------------------------
            if (isset($settings['save_data_in_field']) == true && $settings['save_data_in_field'] == 'yes')
            {
    ...
    public function display_settings($data)
    {
        ...
        if ($field_id < 1) {
            ...
            $vData = $this->EE->image_helper->grabFieldSettings($field_id);
        }
        ...
        // Double check categories just in case
        if (is_array($vData['categories']) === false) {
            $vData['categories'] = explode(',', $vData['categories']);
        }
    ...
    private function _process_post_save($data, $draft_action=NULL)
    {
        ...
        if (isset($this->EE->publisher_lib) === true && (isset($this->EE->publisher_lib->save_status) || isset($this->EE->publisher_lib->publisher_save_status) ) ) {
            $data = (isset($this->EE->session->cache['ChannelImages'])) ? $this->EE->session->cache['ChannelImages']['FieldData'][$this->field_id] : false;

            $save_status = (isset($this->EE->publisher_lib->save_status) === true) ? $this->EE->publisher_lib->save_status : $this->EE->publisher_lib->publisher_save_status;
            if ($save_status == 'draft') {
                $draft_action = 'create';

                if ($this->EE->publisher_entry->has_draft($entry_id)) {
                    $draft_action = 'update';
                }

                // Sometimes we are overwriting an existing draft!
                if ($draft_action == 'update' && $this->EE->publisher_lib->status != 'draft') {
                    $draft_action = 'create';
                }
        ...
        //$this->EE->firephp->log($this->EE->publisher_lib->status);
        ...
        if ($draft_action !== NULL)
        {
            $is_draft = 1;
            $field_id = $this->settings['field_id'];
            $channel_id = isset($this->settings['channel_id']) ? $this->settings['channel_id'] : $this->EE->input->post('channel_id');
        }
        else
        {
            $is_draft = 0;
...
//exit();
    ...
    */
    public function zenbu_display($entry_id, $channel_id, $field_data, $ch_img_data = array(), $field_id, $zenbuSettings, $rules = array(), $upload_prefs = array(), $installed_addons)
    {
        ...
        $settings = $this->EE->image_helper->grabFieldSettings($field_id, $ch_img_data['field_settings'][$field_id]);
